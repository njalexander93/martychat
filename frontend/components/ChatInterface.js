...
const ChatInterface = ({
  ...
  const [messages, setMessages] = useState(welcomeMessage ? [{ role: 'chatbot', content: welcomeMessage }] : []);
  ...
  const compressHistory = (history, maxInteractions) => {
    ...
    const filteredHistory = history.filter(
      (message) => message.role !== 'chatbot' || message.content !== welcomeMessage
    );
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          <a
            ...
            className="p-3 text-text-light hover:bg-white/5 rounded-lg transition-colors"
          >
            Logout
          </a>
        </div>
        ...
        <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-[80%] rounded-lg p-3 whitespace-pre-wrap break-words ${
                  message.role === 'user' ? theme.userMessage : theme.botMessage
                }`}
              >
                {message.content}
              </div>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-gray-200 text-gray-800 rounded-lg p-3">Thinking...</div>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="relative flex flex-col bg-bg-input -ml-4 bg-opacity-10 rounded-xl border border-gray-500 border-opacity-20 shadow-lg overflow-hidden">
              <div className="relative flex">
                <textarea
                  ...
                  className={`flex-1 min-h-[44px] max-h-[${
                    maxInputLines * 16
                  }px] p-3 pr-24 bg-transparent border-none resize-none focus:outline-none focus:ring-0 ${
                    theme.fonts.input
                  }`}
                  onKeyDown={(e) => {
              ...
              <div className="px-3 pb-1.5 text-text-secondary text-[11px]">
                <span className="bg-bg-secondary bg-opacity-20 px-1.5 py-1 text-[10px] rounded-md">
                  <code>Shift + Return</code> for new line
                </span>
              </div>
            </div>
          </form>
